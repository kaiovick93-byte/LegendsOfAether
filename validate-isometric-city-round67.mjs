...
const assetPrep=read('prepare-round67-revision-assets.py');
const groundPrep=read('prepare-round67-ground-assets.py');
const playerPrep=read('prepare-round67-player-assets.py');
const gatePrep=read('prepare-round67-gate-assets.py');
const pkg=JSON.parse(read('package.json'));

expect(pkg.version==='0.2.7','versão do pacote não é 0.2.7');
expect(pkg.scripts?.['validate:isometric-city']==='node validate-isometric-city-round67.mjs','validador do Round 67 não está registrado');
...
expect(scene.includes("id:'artisan', key:'artisan_house', label:'Ateliê de Maelis', u:24.55, v:8.45")&&scene.includes('rect:[23.00,7.05,3.00,2.75]')&&scene.includes('npc:[24.86,9.28]'),'ateliê e Maelis não estão alinhados no corredor marcado junto à muralha Leste');
...
// Muros modulares, portões compactados e cantos laterais com peça única.
const wall='assets/images/environment/isometric/isometric_city_wall.png';
const wallCorner='assets/images/environment/isometric/isometric_city_wall_corner.png';
...
for(const relative of [wall,wallCorner,southGate,eastGate]){
...
if(exists(wallCorner)){
  const d=pngDimensions(wallCorner);
  expect(d.width===256&&d.height===320,'fechamento lateral do canto não preserva o canvas 256x320');
}
...
expect(preload.includes("this.load.image('iso_city_wall_corner'")&&scene.includes('this.addWallCorner(C.CITY_MIN,C.CITY_MAX)')&&scene.includes('this.addWallCorner(C.CITY_MAX,C.CITY_MIN)'),'cantos esquerdo/direito não usam fechamento único');
expect((scene.match(/this\.addWallCorner\(/g)||[]).length===2,'quantidade inesperada de fechamentos laterais');
expect(gatePrep.includes('left, right = 205, 1080')&&gatePrep.includes('extensions=removed'),'remoção reproduzível das extensões dos portões está ausente');
expect(exists('assets/source/city-gates/isometric_city_gate_full.png')&&exists('assets/source/city-gates/isometric_city_gate_east_full.png'),'fontes integrais dos portões não foram preservadas');
...
expect(playerPrep.includes('PLAYER_ATLASES_OK heroes=')&&playerPrep.includes('variants=4 directions=8 frames=4 outlines=24')&&playerPrep.includes('NORTH_OVERRIDES'),'preparo reproduzível das folhas do jogador está incompleto');
...
expect(assetPrep.includes('forja em ardósia de carvão')&&assetPrep.includes('arquivo em ameixa')&&assetPrep.includes('telha marrom rústica'),'identidades distintas dos telhados não estão documentadas no preparo visual');
expect(assetPrep.includes('BUILDING_DIR / "blacksmith_shop.png", 350, 18')&&assetPrep.includes('BUILDING_DIR / "scholar_house.png", 610, 202')&&assetPrep.includes('BUILDING_DIR / "tavern_house.png", 335, 20'),'recoloração distinta dos três telhados não está reproduzível');
expect(groundPrep.includes('source_crop=(470, 230, 1306, 648)')&&groundPrep.includes('isometric_residential_ground.png')&&groundPrep.includes('isometric_grass_tufts.png'),'preparo reproduzível do gramado ou bairro está incompleto');
...
if(issues.length){
  console.error('ISOMETRIC_CITY_ROUND67_AUDIT_FAILED');
  for(const issue of issues)console.error(`- ${issue}`);
  process.exit(1);
}

console.log('ISOMETRIC_CITY_ROUND67_AUDIT_OK walls=2side-corners gates=compact-no-overlap returns=directional collision=body+alpha ground=refined+16sway residential=4paved-yards+blue-inset merchant=plaza-facing artisan=marked-east-line heroes=6x4statesx8dirs equipment=class-locked occlusion=48outlines mira=action general=opposite+action guards=inward roofs=distinct walkers=2x8dirs actions=10x4 smoke=9animated scholar=compact healer=devastated+restored prompts=stacked speech=active docs=consolidated prototype=removed');
