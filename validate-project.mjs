...
expect(scene.includes("sourceMinY:.58, minHits:2")&&scene.includes("registerSolidMask(bench, 'city_bench'"),'árvores extras ou bancos não usam base física pequena');
...
for(const station of [
  "{wall:'south',lane:6.35", "{wall:'south',lane:10.15", "{wall:'south',lane:18.10",
  "{wall:'south',lane:21.65", "{wall:'east',lane:11.05", "{wall:'east',lane:18.55"
])expect(lowerWallSiege.includes(station),`estação visual do cerco ausente: ${station}`);
expect(lowerWallSiege.includes('v:25.62,z:44')&&lowerWallSiege.includes('u:25.62,v:spec.lane,z:44'),'arqueiros não estão posicionados no parapeito dos muros inferiores');
...
expect(lowerWallSiege.includes('corpse.setIsoPosition(attackIso.u,attackIso.v,0).setVisible(true)')&&lowerWallSiege.includes('removeCorpseWhenAttackBegins')&&lowerWallSiege.includes('this.schedule(650+station.index%3*140,()=>this.spawnGoblin(station))'),'goblin derrotado não mantém cadáver e reinicia o ciclo infinito');
...
const siegeZoneBlocked=(u,v,radius=.27)=>{
  const stations=[[6.35,26.82],[10.15,26.82],[18.10,26.82],[21.65,26.82],[26.82,11.05],[26.82,18.55]];
...
expect(scene.includes('const stations=[')&&scene.includes('{u:6.35,v:26.82}')&&scene.includes('{u:26.82,v:18.55}'),'seis zonas físicas locais do cerco não foram declaradas');
expect(siegeZoneBlocked(21.65,26.82),'estação sul permite alcançar os goblins');
expect(!siegeZoneBlocked(14,25.1),'corredor do Portão Sul foi fechado pela barreira do cerco');
expect(siegeZoneBlocked(26.82,18.55),'estação leste permite alcançar os goblins');
...
expect(scene.includes("label: 'fonte', mode:'foundation', sourceMinY:.45")&&scene.includes("label: building.label, mode: 'foundation'"),'fonte ou estabelecimentos não usam somente a fundação física');
expect(scene.includes('getFoundationRowSpans(entry,targetMask)')&&scene.includes("if(entry.mode!=='foundation')return false"),'vãos transparentes internos das fachadas não são fechados na faixa de fundação');
...
expect(elderStats.length===4&&elderStats.every(cell=>cell.height===198&&cell.minY===18&&cell.maxY===215),'velhinho não mantém escala menor e pés ancorados nos quatro quadros');
...
expect(minimap.includes('setDisplaySize(216,162)')&&minimap.includes("this.projection==='aether-territory'")&&minimap.includes('this.localZoom=3.25')&&minimap.includes('this.art.setCrop')&&minimap.includes('216/cropW,162/cropH')&&minimap.includes('this.playerHalo'),'minimapa não usa recorte local proporcional do território e marcador do jogador');
...
const continuousWallBlocked=(u,v,r=.27)=>{
  ...
  const eastGate=v-r>13.42&&v+r<14.58,southGate=u-r>13.42&&u+r<14.58;
  return (overlaps(u,1.18,3.12)&&along(v))||(overlaps(v,1.18,3.12)&&along(u))||
...
expect(territory.includes("this.addIsoImage('outskirts_aether_sign_v2',12.4,35.7,170")&&territory.includes("this.addIsoImage('farm_empty_wagon',9.4,52.4,118"),'Estrada Velha não possui sinalização, marco e restos de carroça');
...
const waterBlocked=(u,v,r=.27)=>{
  for(const [cu,cv,cr] of [[43.8,21,2.15],[45.7,40.1,2.2]])if((u-cu)**2+(v-cv)**2<=(cr+r)**2)return false;
  if(((u-52)/(6.6+r))**2+((v-50.2)/(5.4+r))**2<=1)return true;
