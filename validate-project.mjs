...
expect(preload.includes("this.load.image('merchant_shop'")&&preload.includes("this.load.image('iso_city_pavement_v2'"),'novas artes não usam as chaves estáveis do preload');
const pavementV2='assets/images/environment/isometric/isometric_city_pavement_v2.png';
const cityStreets='assets/images/environment/isometric/isometric_city_streets.png';
for(const relative of [pavementV2,cityStreets]){
  expect(exists(relative),`camada urbana v11.9 ausente: ${relative}`);
  if(exists(relative)){
    const d=pngDimensions(relative);
    expect(d.width===2304&&d.height===1152&&pngColorType(relative)===6,`camada urbana não preserva canvas RGBA 2304x1152: ${relative}`);
  }
}
expect(preload.includes("'iso_city_pavement_v2'")&&preload.includes("'iso_city_streets'")&&scene.includes("'iso_city_pavement_v2'")&&scene.includes("'iso_city_streets'"),'ruas e calçadas exatas não estão ligadas ao piso da cidade');
...
expect(scene.includes('const targetHeight=154')&&scene.includes('depthOffset:106'),'pilar lateral não preserva escala uniforme ou não cobre as duas pontas em profundidade');
...
for(const station of [
  "{wall:'south',lane:6.20", "{wall:'south',lane:9.45", "{wall:'south',lane:18.55",
  "{wall:'south',lane:21.80", "{wall:'east', lane:7.10", "{wall:'east', lane:20.85"
])expect(lowerWallSiege.includes(station),`estação visual do cerco ausente: ${station}`);
expect(lowerWallSiege.includes("?{u:spec.lane,v:25.55,z:78}")&&lowerWallSiege.includes(":{u:25.55,v:spec.lane,z:78}"),'arqueiros não estão posicionados no topo dos muros inferiores');
expect(lowerWallSiege.includes(".setData('decorativeOnly',true)")&&lowerWallSiege.includes('archer.disableInteractive()')&&lowerWallSiege.includes('goblin.disableInteractive()'),'atores do cerco não estão marcados como cenográficos e não interativos');
...
expect(lowerWallSiege.includes('this.schedule(620+(station.index%3)*120,()=>this.spawnGoblin(station))'),'goblin derrotado não reinicia o ciclo infinito');
expect(lowerWallSiege.includes('Math.max(archer.depth,goblin.depth)+2')&&lowerWallSiege.includes('depthBase:this.config.depthBase,depthOffset'),'flechas e atores não respeitam a profundidade isométrica');
...
const siegeZoneBlocked=(u,v,radius=.27)=>{
  const limit=25.22;
  const insideEastGate=v-radius>13.42&&v+radius<14.58;
  const insideSouthGate=u-radius>13.42&&u+radius<14.58;
  return (u+radius>limit&&!insideEastGate)||(v+radius>limit&&!insideSouthGate);
};
...
expect(scene.includes('const limit=25.22')&&scene.includes('insideEastGate')&&scene.includes('insideSouthGate'),'barreira do cerco não preserva somente os dois vãos de portão');
expect(siegeZoneBlocked(20,25.1),'muro sul permite alcançar os goblins');
expect(!siegeZoneBlocked(14,25.1),'corredor do Portão Sul foi fechado pela barreira do cerco');
expect(siegeZoneBlocked(25.1,20),'muro leste permite alcançar os goblins');
...
spawnTargets.push(
  ...
  targetGeometry('assets/images/characters/npcs/isometric/guard_iso.png',24.65,11.75,124/224,{offsetY:8,originY:1}),
  targetGeometry('assets/images/characters/npcs/isometric/south_guard_iso.png',11.75,24.65,124/224,{offsetY:8,originY:1,flipX:true})
);
...
expect(travelerStats.length===32&&travelerStats.every(cell=>cell.height>=188&&cell.height<=219),'viajante não mantém escala corporal estável nos 32 quadros');
...
expect(bottomBar.includes("10:'K',11:'I',12:'M',13:'C',14:'P'")&&bottomBar.includes("this.slotTexts[12].setText('MAPA')")&&bottomBar.includes("this.slotTexts[14].setText('MENU')"),'comandos K/I/M/C/P não aparecem após as oito habilidades');
...
expect(preload.includes("this.load.image('city_map_exact_2_5d'")&&scene.includes("mapTexture: 'city_map_exact_2_5d', mapProjection: 'aether-city-exact'"),'mapa exato 2,5D não está ligado à Cidade de Aether');
expect(minimap.includes('setDisplaySize(216,162)')&&minimap.includes("this.projection==='aether-city-exact'")&&minimap.includes('offsetY=(768-this.h*mapScale)/2')&&minimap.includes('this.playerHalo'),'minimapa não usa arte 4:3, projeção exata e marcador do jogador');
expect(mapPanel.includes("this.projection==='aether-city-exact'")&&mapPanel.includes("'MAPA — CIDADE DE AETHER'")&&mapPanel.includes('offsetY=(768-this.worldHeight*mapScale)/2')&&mapPanel.includes('this.mapPanel')===false,'painel de mapa exato está ausente ou contém dependência circular');
...
console.log('PROJECT_AUDIT_OK movement=6heroesx8dirsx2gates contact=opaque-feet+escape iso=authoritative camera=north-margin viewport=native inventory=front npc-prompt=white-unified waystone=white-dialogue hud=2items+8skills+5commands map=city-2.5d sprites=grounded+mirrored-pairs walls=seamless-side-pillars gates=center-only siege=visual-only+blocked occlusion=outline-only assets=runtime-clean');
