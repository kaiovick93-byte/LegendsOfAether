...
// B4.0C2: os três materiais compactos deixam de cobrir todo o mapa como um
// TileSprite. Eles agora entram em pequenos pontos próximos da Estrada Velha,
// evitando que o próprio polimento crie uma nova repetição em escala grande.
// As posições são relativas a módulos já aprovados; não há coordenada de rota
// nova nem alteração do losango lógico U/V 0…82.
const OUTSKIRTS_B4_GROUND_POLISH_PLAN=Object.freeze([
  {texture:'outskirts_ground_b4_variation_0',module:2,offset:{x:-118,y:8},scale:.34,alpha:.84,flipX:false},
  {texture:'outskirts_ground_b4_variation_1',module:3,offset:{x:116,y:-5},scale:.33,alpha:.82,flipX:true},
  {texture:'outskirts_ground_b4_variation_2',module:5,offset:{x:-120,y:7},scale:.32,alpha:.82,flipX:false},
  {texture:'outskirts_ground_b4_variation_0',module:7,offset:{x:118,y:-8},scale:.35,alpha:.84,flipX:true},
  {texture:'outskirts_ground_b4_variation_1',module:9,offset:{x:-116,y:9},scale:.32,alpha:.82,flipX:false},
  {texture:'outskirts_ground_b4_variation_2',module:10,offset:{x:117,y:-6},scale:.33,alpha:.82,flipX:true},
  {texture:'outskirts_ground_b4_variation_0',module:12,offset:{x:-118,y:5},scale:.33,alpha:.84,flipX:false},
  {texture:'outskirts_ground_b4_variation_1',module:13,offset:{x:116,y:-7},scale:.34,alpha:.82,flipX:true},
  {texture:'outskirts_ground_b4_variation_2',module:14,offset:{x:-112,y:6},scale:.32,alpha:.82,flipX:false}
]);
...
// B4.0C2: os PNGs C1B são peças compactas, portanto são colocados por um
// manifest visual explícito em vez de assumir indevidamente o canvas de cada
// conector. Todas as medidas abaixo são offsets de tela já em pixels de mundo.
// Não são conectores, não participam de colisão e não mudam a largura jogável.
const OLD_ROAD_POLISH_LAYOUT=Object.freeze({
  straightLong:[
    {texture:'old_road_polish_straight_long_a_v1',offset:{x:-47,y:0},scale:.39,alpha:.99,wobbleY:17},
    {texture:'old_road_polish_straight_long_b_v1',offset:{x:47,y:0},scale:.39,alpha:.99,wobbleY:-17},
    {texture:'old_road_polish_straight_long_c_v1',offset:{x:0,y:2},scale:.40,alpha:.97,phases:[0,1,2]}
  ],
  straightShort:[
    {texture:'old_road_polish_straight_short_a_v1',offset:{x:0,y:-6},scale:.30,alpha:.98},
    {texture:'old_road_polish_straight_short_b_v1',offset:{x:24,y:23},scale:.35,alpha:.98}
  ],
  yJunction:[
    {texture:'old_road_polish_y_junction_a_v1',offset:{x:0,y:0},scale:.65,alpha:.99}
  ],
  startRun:[
    {texture:'old_road_polish_start_run_a_v1',offset:{x:-22,y:8},scale:.56,alpha:.99}
  ],
  gateTransition:[
    {texture:'old_road_polish_gate_transition_a_v1',offset:{x:-32,y:-48},scale:.62,alpha:.99}
  ]
});
// As curvas da rota B3.8 são uma sequência de bocas retas com ângulos
// graduais. Estes acentos ficam 54px para dentro da peça seguinte: a boca em
// si permanece limpa, enquanto borda interna e externa deixam de parecer um
// recorte verde contínuo.
const OLD_ROAD_CURVE_POLISH_ACCENTS=Object.freeze([
  {texture:'old_road_polish_curve_right_a_v1',offset:{x:-50,y:54},scale:.52,alpha:.99},
  {texture:'old_road_polish_curve_right_b_v1',offset:{x:50,y:54},scale:.49,alpha:.99}
]);
// Pequenos recortes da própria variação de ground entram somente em trechos
// alternados da borda. Eles cobrem partes da faixa verde-base sem invadir o
// centro caminhável e impedem a leitura de duas linhas paralelas perfeitas.
const OLD_ROAD_EDGE_GROUND_ACCENTS=Object.freeze([
  {texture:'outskirts_ground_b4_variation_0',offset:{x:-60,y:-10},scale:.28,alpha:.90,period:3,phase:0},
  {texture:'outskirts_ground_b4_variation_1',offset:{x:60,y:10},scale:.27,alpha:.90,period:3,phase:1},
  {texture:'outskirts_ground_b4_variation_2',offset:{x:-60,y:12},scale:.26,alpha:.90,period:3,phase:2}
]);
...
export class AetherTerritory{
  constructor(scene,config){
    ...
    this.roadPolishOrdinal=0;this.roadPolishPlacements=[];this.groundPolishPlacements=[];
  ...
  createGroundMosaic(){
    ...
    // As três variações C1B são aplicadas mais abaixo, junto da rota, como
    // detalhes pontuais. Elas não são mais repetidas no mapa inteiro.
  ...
  transformRoadPolishOffset(offset={},rotation=0,flipX=false,flipY=false){
    let x=offset.x??0,y=offset.y??0;
    if(flipX)x=-x;if(flipY)y=-y;
    const cos=Math.cos(rotation),sin=Math.sin(rotation);
    return{x:x*cos-y*sin,y:x*sin+y*cos};
  }

  addRoadPolishOverlay(texture,anchorScreen,options={}){
    const rotation=options.rotation??0,flipX=!!options.flipX,flipY=!!options.flipY;
    const offset=this.transformRoadPolishOffset(options.offset,rotation,flipX,flipY);
    const point=this.screenToLogical(anchorScreen.x+offset.x,anchorScreen.y+offset.y);
    const sprite=this.addGround(texture,point.u,point.v,options.scale??OLD_ROAD_CONNECTOR_STANDARD.scale,options.layer??(OLD_ROAD_PROTOTYPE_LAYERS.ROAD+.12),{
      rotation,flipX,flipY,alpha:options.alpha??1,visibleRadius:options.visibleRadius??29,activeRadius:options.activeRadius??34
    });
    if(sprite){
      const kind=options.kind??'road-polish';
      sprite.setData?.('aetherRenderClass',kind);
      const placement={texture,kind,point,anchor:{x:anchorScreen.x,y:anchorScreen.y},offset:{...(options.offset??{})},scale:options.scale??OLD_ROAD_CONNECTOR_STANDARD.scale,rotation,flipX,flipY};
      if(kind==='ground-polish')this.groundPolishPlacements.push(placement);else this.roadPolishPlacements.push(placement);
    }
    return sprite;
  }

  applyRoadPolishForModule(moduleName,module,ordinal){
    const layout=OLD_ROAD_POLISH_LAYOUT[moduleName]??[];
    const sprites=[];
    for(const placement of layout){
      const phases=placement.phases;
      if(phases&&!phases.includes(ordinal%4))continue;
      const wobble=(ordinal%3)-1;
      const offset={...(placement.offset??{})};
      if(placement.wobbleX)offset.x+=(placement.wobbleX*wobble);
      if(placement.wobbleY)offset.y+=(placement.wobbleY*wobble);
      const sprite=this.addRoadPolishOverlay(placement.texture,module.center,{...placement,offset,
        rotation:module.rotation,flipX:module.flipX,flipY:module.flipY,kind:'road-polish'
      });
      if(sprite)sprites.push(sprite);
    }
    if(moduleName==='straightLong'){
      OLD_ROAD_EDGE_GROUND_ACCENTS.forEach(placement=>{
        if(ordinal%placement.period!==placement.phase)return;
        const sprite=this.addRoadPolishOverlay(placement.texture,module.center,{...placement,
          rotation:module.rotation,flipX:module.flipX,flipY:module.flipY,kind:'road-polish'
        });
        if(sprite)sprites.push(sprite);
      });
  ...
  applyCurvePolishAccents(modules){
    // Esses índices são inflexões internas da rota já aprovada; cada acento
    // é deslocado para o centro da peça seguinte e nunca cruza uma boca.
    [3,7,11].forEach(index=>{
      const module=modules[index];
      const anchor=module?.connectors?.north;
      if(!anchor)return;
      OLD_ROAD_CURVE_POLISH_ACCENTS.forEach(placement=>this.addRoadPolishOverlay(placement.texture,anchor,{...placement,
        rotation:module.rotation,flipX:module.flipX,flipY:module.flipY,kind:'road-polish'
      }));
    });
  }

  applyGroundPolishNearRoad(modules){
    OUTSKIRTS_B4_GROUND_POLISH_PLAN.forEach(spec=>{
      const module=modules[spec.module];
      if(!module)return;
      this.addRoadPolishOverlay(spec.texture,module.center,{...spec,
        rotation:module.rotation,flipX:!!spec.flipX!==!!module.flipX,flipY:module.flipY,
        layer:OLD_ROAD_PROTOTYPE_LAYERS.TERRAIN+.14,kind:'ground-polish',visibleRadius:29,activeRadius:34
      });
    });
  }
  ...
  placeRoadConnectorModule(moduleName,anchorScreen,connector,options={}){
    ...
    module.polishSprites=this.applyRoadPolishForModule(moduleName,module,this.roadPolishOrdinal++);
    return module;
  }
  ...
  createOldRoadPrototype(){
    ...
    this.applyCurvePolishAccents(oldRoadModules);
    this.applyGroundPolishNearRoad(oldRoadModules);
    ...
    this.scene.registry.set('oldRoadPolishApplication',{
      mode:'explicit-compact-overlays-c1b',
      roadTextures:[...new Set(this.roadPolishPlacements.map(item=>item.texture))],
      groundTextures:[...new Set(this.groundPolishPlacements.map(item=>item.texture))],
      roadPlacements:this.roadPolishPlacements.length,groundPlacements:this.groundPolishPlacements.length
    });
  }
