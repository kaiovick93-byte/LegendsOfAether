...
export class AetherTerritory{
  ...
  createOldRoadGround(){
    ...
    const shoulders=[
      [5.2,74.4,.72,.08],[10.2,74.1,.68,-.12],[5.6,68.5,.69,.16],[10.8,67.1,.70,-.18],
      [5.4,61.7,.68,.09],[12.1,60.2,.72,-.1],[6.1,54.8,.70,.15],[13.2,52.8,.68,-.14],
      [7.2,47.6,.70,.1],[14.6,45.5,.69,-.12],[8.2,40.8,.68,.15],[15.8,38.8,.72,-.1],
      [9.5,34.4,.70,.1],[16.2,32.6,.68,-.16]
    ];
    shoulders.forEach(([u,v,scale,rotation],index)=>this.addGround(kit.grass,u,v,scale,rotation,AETHER_TERRITORY_RENDER_LAYERS.TERRAIN,{flipX:index%3===1,alpha:.92,visibleRadius:28}));

    const cycle=[kit.straight,kit.damaged,kit.straight,kit.transition,kit.straight,kit.curve,kit.damaged];
    let stampIndex=0;
    for(let index=1;index<AETHER_OLD_ROAD.length;index++){
      const a=AETHER_OLD_ROAD[index-1],b=AETHER_OLD_ROAD[index];
      const length=Math.hypot(b.u-a.u,b.v-a.v),steps=Math.max(1,Math.ceil(length/2.65));
      ...
      for(let step=0;step<steps;step++){
        ...
        const atBend=step===0&&[3,5,7].includes(index);
        const atGate=index===AETHER_OLD_ROAD.length-1&&step===steps-1;
        const key=atGate?kit.transition:atBend?kit.curve:cycle[stampIndex%cycle.length];
        const scale=key===kit.curve ? .69 : key===kit.junction ? .70 : .72+(stampIndex%3)*.012;
        this.addGround(key,u,v,scale,rotation+(stampIndex%2?.014:-.014),AETHER_TERRITORY_RENDER_LAYERS.ROAD,{flipX:stampIndex%4===1,alpha:.96,visibleRadius:30});
        stampIndex++;
      }
    }
    // Pequena saída lateral antes de a estrada alcançar Aether: é uma leitura
    // visual de bifurcação, não cria rota narrativa ou passagem nova.
    this.addGround(kit.junction,12.78,35.15,.70,-.20,AETHER_TERRITORY_RENDER_LAYERS.ROAD,{alpha:.97,visibleRadius:30});
  }
