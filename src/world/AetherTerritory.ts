...
const OLD_ROAD_PROTOTYPE=Object.freeze({
  anchor:{u:26.5,v:78.0},scale:.42,
  groundOffsets:[
    [-143,130],[143,130],[-286,50],[0,50],[286,50],[-143,-30],[143,-30],
    [-286,-110],[0,-110],[286,-110],[-123,-190],[163,-190],[-241,-270],
    [45,-270],[331,-270],[-68,-350],[218,-350],[-198,-430],[88,-430],
    [374,-430],[-55,-510],[231,-510],[-198,-590],[88,-590],[374,-590],
    [-55,-670],[231,-670]
  ],
  roadOffsets:[
    ['straight',0,0],['straight',0,-145],['curve',44.5,-343],['junction',88.2,-523]
  ]
});
...
export class AetherTerritory{
  ...
  /** Protótipo 9D-B3.2: apenas Ground + 2 Straight + Curve + Y-Junction. */
  createOldRoadPrototype(){
    const prototype=OLD_ROAD_PROTOTYPE,kit=OLD_ROAD_PROTOTYPE_KIT;
    const halfWidth=(this.config.tileWidth??96)/2,halfHeight=(this.config.tileHeight??48)/2;
    const atOffset=(x,y)=>({
      u:prototype.anchor.u+(x/halfWidth+y/halfHeight)/2,
      v:prototype.anchor.v+(y/halfHeight-x/halfWidth)/2
    });
    prototype.groundOffsets.forEach(([x,y])=>{
      const point=atOffset(x,y);
      this.addGround(kit.ground,point.u,point.v,prototype.scale,OLD_ROAD_PROTOTYPE_LAYERS.TERRAIN);
    });
    prototype.roadOffsets.forEach(([piece,x,y])=>{
      const point=atOffset(x,y);
      this.addGround(kit[piece],point.u,point.v,prototype.scale,OLD_ROAD_PROTOTYPE_LAYERS.ROAD);
    });
  }
