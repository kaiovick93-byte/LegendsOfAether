...
import {
  AETHER_EAST_MAIN_ROAD,AETHER_FUTURE_NEW_GAME_SPAWN,AETHER_LAKE,AETHER_LANDMARKS,
...
export class AetherTerritory{
  ...
  createGroundMosaic(){
    ...
    for(let u=3.9;u<=82;u+=7.8)for(let v=3.9;v<=82;v+=7.8){
      const p=this.project(u,v),tile=this.scene.add.image(p.x,p.y,'outskirts_ground_tile_v2').setOrigin(.5).setScale(1.025).setDepth(this.config.depthBase-80);
  ...
  nearCrossing(u,v,padding=1.35){return AETHER_STREAM.crossings.some(c=>(u-c.u)**2+(v-c.v)**2<=padding**2)}
  ...
  stampPolyline(points,scale,spacing=2.05){
    for(let index=1;index<points.length;index++){
      ...
      for(let step=0;step<steps;step++){
        ...
        this.addFlat('outskirts_old_road_v2',u,v,scale,rotation,-43,{visibleRadius:30});
      }
    }
  }
  ...
  stampStream(){
    for(let index=1;index<AETHER_STREAM.points.length;index++){
      ...
      for(let step=0;step<steps;step++){
        const t=(step+.5)/steps,u=Phaser.Math.Linear(a.u,b.u,t),v=Phaser.Math.Linear(a.v,b.v,t);
        const water=this.addFlat('outskirts_stream_v2',u,v,.47,rotation,-44,{flipX:true,visibleRadius:31,alpha:.96});
  ...
  createWaterSystem(){
    ...
    for(const crossing of AETHER_STREAM.crossings){
      this.addIsoImage('outskirts_bridge_v2',crossing.u,crossing.v,220,{occluder:false,depthOffset:-.12,visibleRadius:31});
    }
  ...
  createOldRoadIdentity(){
    ...
    this.addIsoImage('farm_empty_wagon',9.4,52.4,118,{solid:{label:'resto de carroça',width:98,height:20,yOffset:-7},flipX:true,visibleRadius:28});
  ...
  isLogicalBarrierBlocked(u,v,radius=.27){
    ...
    const woodsU=(u-72.15)/(3.65+radius),woodsV=(v-48.55)/(2.75+radius);
