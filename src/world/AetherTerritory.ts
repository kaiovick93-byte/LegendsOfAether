...
const OLD_ROAD_GROUND_KIT=Object.freeze({
  baseA:'old_road_ground_base_a_v4',baseB:'old_road_ground_base_b_v4',wornGround:'old_road_ground_worn_v4',
  connectorLong:'old_road_connector_long_v4',connectorShort:'old_road_connector_short_v4',
  straight:'old_road_straight_v3',curve:'old_road_curve_v3',junction:'old_road_junction_v3',
  transition:'old_road_transition_v3',damaged:'old_road_damaged_v3',grass:'old_road_grass_v3',
  creekBank:'old_road_creek_bank_v3',embankment:'old_road_embankment_v3'
});
...
export class AetherTerritory{
  ...
  /**
   * Composição própria da Estrada Velha. Os módulos continuam sendo ground
   * estável: a rota não recebe profundidade baseada em u+v e nunca encobre
   * jogadores, criaturas ou o cenário alto separado.
   */
  createOldRoadGround(){
    const kit=OLD_ROAD_GROUND_KIT;
    // B3.1: a Estrada Velha não é uma sucessão de “ilhas” transparentes.
    // Este tapete usa diamantes isométricos de 8×8 lógicos, sobrepostos de
    // leve, e cobre todo o corredor jogável e suas laterais próximas. Os
    // módulos são fonte de variação visual, não recortes soltos.
    const baseColumns=[3.9,11.9,19.9];
    const baseRows=[27.9,35.9,43.9,51.9,59.9,67.9,75.9,83.9];
    baseRows.forEach((v,row)=>baseColumns.forEach((u,column)=>{
      const index=row*baseColumns.length+column;
      const key=index%2?kit.baseA:kit.baseB;
      this.addGround(key,u,v,1.022,0,AETHER_TERRITORY_RENDER_LAYERS.TERRAIN,{
        flipX:index%3===1,alpha:1,visibleRadius:38
      });
    }));
    // Duas manchas pequenas de terra batida quebram a regularidade do gramado
    // sem voltar a formar grandes ilhas de textura.
    [[5.45,72.7],[17.05,48.6]].forEach(([u,v],index)=>this.addGround(
      kit.wornGround,u,v,.27,0,AETHER_TERRITORY_RENDER_LAYERS.TERRAIN,{flipX:index===1,alpha:.42,visibleRadius:34}
    ));

    // O eixo vertical da arte do conector é alinhado com o vetor da estrada.
    // No B3 o ângulo era aplicado ao eixo horizontal do Sprite; isso deixava
    // segmentos perpendiculares e intensificava a impressão de fragmentos.
    const roadRotation=(a,b)=>{
      const screenA=this.project(a.u,a.v),screenB=this.project(b.u,b.v);
      return Math.atan2(screenB.y-screenA.y,screenB.x-screenA.x)-Math.PI/2;
    };
    const connectorScale=.272;
    for(let index=1;index<AETHER_OLD_ROAD.length;index++){
      ...
      // Cada conector longo mede ~424 px nesta escala. A distância entre
      // centros fica menor que 285 px, portanto nunca há buraco entre saídas.
      const steps=Math.max(1,Math.ceil(screenLength/282));
      const rotation=roadRotation(a,b);
      for(let step=0;step<steps;step++){
        const t=(step+.5)/steps,u=Phaser.Math.Linear(a.u,b.u,t),v=Phaser.Math.Linear(a.v,b.v,t);
        const key=steps===1?kit.connectorLong:step===0||step===steps-1?kit.connectorShort:kit.connectorLong;
        this.addGround(key,u,v,connectorScale,rotation,AETHER_TERRITORY_RENDER_LAYERS.ROAD,{visibleRadius:34});
      }
    }

    // São detalhes sobre a malha conectada, nunca seus únicos trechos. As
    // peças B3 boas permanecem em uso como curvas, variações e acabamento.
    const accents=[
      [kit.transition,7.7,75.4,.42,roadRotation(AETHER_OLD_ROAD[0],AETHER_OLD_ROAD[1])],
      [kit.straight,8.0,66.2,.43,roadRotation(AETHER_OLD_ROAD[1],AETHER_OLD_ROAD[2])],
      [kit.damaged,9.15,57.0,.40,roadRotation(AETHER_OLD_ROAD[2],AETHER_OLD_ROAD[3])],
      [kit.curve,10.15,52.9,.45,roadRotation(AETHER_OLD_ROAD[3],AETHER_OLD_ROAD[4])],
      [kit.straight,10.95,47.1,.42,roadRotation(AETHER_OLD_ROAD[4],AETHER_OLD_ROAD[5])],
      [kit.damaged,11.85,41.1,.40,roadRotation(AETHER_OLD_ROAD[5],AETHER_OLD_ROAD[6])],
      [kit.curve,12.85,34.95,.45,roadRotation(AETHER_OLD_ROAD[6],AETHER_OLD_ROAD[7])],
      [kit.transition,13.72,29.55,.43,roadRotation(AETHER_OLD_ROAD[8],AETHER_OLD_ROAD[9])]
    ];
    accents.forEach(([key,u,v,scale,rotation],index)=>this.addGround(key,u,v,scale,rotation,AETHER_TERRITORY_RENDER_LAYERS.ROAD,{
      flipX:index%3===1,alpha:.74,visibleRadius:34
    }));
    // Pequena saída lateral antes de a estrada alcançar Aether: leitura de
    // bifurcação, sem abrir rota, colisão ou evento narrativo adicional.
    this.addGround(kit.junction,12.78,35.15,.48,-.20,AETHER_TERRITORY_RENDER_LAYERS.ROAD,{alpha:.78,visibleRadius:34});
  }
  ...
  createRoadNetwork(){
    // A Estrada Velha é montada pelo kit modular acima. As demais rotas mantêm
    // seus módulos atuais até receberem o próprio passe visual.
  ...
  createVegetation(){
    // A Estrada Velha recebe carvalhos próprios, no mesmo acabamento do kit
    // modular. Tronco/footprint e copa/oclusão continuam no IsoSprite.
    const oldRoadTrees=[
      [8,33,174],[18.5,31.5,166],[7,39.5,180],[18,38.8,170],[5.2,48.5,184],[16,50.5,170],
      [4.8,58,180],[14.2,61,166],[4.5,72,184],[12.5,75,178],[18,44,164]
    ];
    oldRoadTrees.forEach(([u,v,height],index)=>this.addIsoImage('old_road_mature_oak_v3',u,v,height,{solid:{label:'tronco de carvalho',width:27+height*.035,height:14+height*.018,yOffset:-6},flipX:index%3===1,visibleRadius:34}));
    const trees=[
      [34.5,55.5,170],
