...
export class LowerWallSiege {
  constructor(scene,config){this.scene=scene;this.config=config;this.active=true;this.regionActive=true;this.stations=[];this.timers=[];this.tweens=[];this.effects=[];this.ensureArrowTexture();this.createStations()}
  ...
  createStations(){[
    {wall:'south',lane:6.35,delay:180},{wall:'south',lane:10.15,delay:960},{wall:'south',lane:18.10,delay:430},{wall:'south',lane:21.65,delay:1410},
    {wall:'east',lane:11.05,delay:650},{wall:'east',lane:18.55,delay:1180}
  ].forEach((spec,index)=>this.createStation(spec,index))}
  createStation(spec,index){
    const south=spec.wall==='south',archerIso=south?{u:spec.lane,v:25.62,z:44}:{u:25.62,v:spec.lane,z:44},attackIso=south?{u:spec.lane,v:26.82}:{u:26.82,v:spec.lane},spawnIso=south?{u:spec.lane,v:27.72}:{u:27.72,v:spec.lane},wallIso=south?{u:spec.lane,v:26}:{u:26,v:spec.lane};
    const archer=this.addIsoSprite('siege_archer',archerIso,112,.20).setFlipX(south).setData('decorativeOnly',true);
    const goblin=this.addIsoSprite('siege_goblin',spawnIso,102,.18).setFlipX(south).setData('decorativeOnly',true).setVisible(false);
    const corpse=this.addIsoSprite('siege_goblin',attackIso,102,.17).setFlipX(south).setData('decorativeOnly',true).setVisible(false);
  ...
  addIsoSprite(texture,position,targetHeight,depthOffset){const source=this.scene.textures.get(texture).getSourceImage();return new IsoSprite({scene:this.scene,isoX:position.u,isoY:position.v,isoZ:position.z||0,texture,tileWidth:this.config.tileWidth,tileHeight:this.config.tileHeight,screenOriginX:this.config.originX,screenOriginY:this.config.originY,depthBase:this.config.depthBase,depthOffset}).setScale(targetHeight/source.height)}
  ...
  startArcherGuard(station){const archer=station.archer;if(!this.active||!this.regionActive||!archer?.active)return;this.disposeTween(station.archerBob);station.archerBob=this.addTween({targets:archer,y:archer.y-1.5,duration:1180+(station.index%3)*170,ease:'Sine.InOut',yoyo:true,repeat:-1})}
  spawnGoblin(station){
    ...
    goblin.setIsoPosition(spawnIso.u,spawnIso.v,0).setVisible(true).setAlpha(0).clearTint().setAngle(0).setScale(goblinScale*.82);
  ...
  goblinStrike(station,strike){
    if(!this.active||!this.regionActive||!station.goblin?.visible)return;const goblin=station.goblin,base={x:goblin.x,y:goblin.y},wallPoint=this.config.project(station.wallIso.u,station.wallIso.v),dx=(wallPoint.x-base.x)*.17,dy=(wallPoint.y+12-base.y)*.17,angle=station.spec.wall==='south'?-7:7;
  ...
  archerPrepareAndFire(station){
    if(!this.active||!this.regionActive||!station.archer?.active||!station.goblin?.visible)return;const archer=station.archer,side=station.spec.wall==='south'?-1:1,base={x:archer.x,y:archer.y};this.disposeTween(station.archerBob);station.archerBob=null;this.scene.tweens.killTweensOf(archer);
    this.addTween({targets:archer,x:base.x+side*2,y:base.y-1,angle:side*-2,duration:150,ease:'Sine.Out',onComplete:()=>this.addTween({targets:archer,x:base.x+side*5,y:base.y-3,angle:side*-5,duration:180,ease:'Quad.Out',onComplete:()=>{this.fireArrow(station);this.addTween({targets:archer,x:base.x,y:base.y,angle:0,duration:250,ease:'Sine.InOut',onComplete:()=>this.startArcherGuard(station)})}})});
  }
  fireArrow(station){
  ...
  goblinReactAndDie(station){if(!this.active||!this.regionActive||!station.goblin?.visible)return;const goblin=station.goblin,base={x:goblin.x,y:goblin.y};goblin.setTint(0xffdf9c);this.addTween({targets:goblin,x:base.x+(station.spec.wall==='south'?7:-7),duration:85,yoyo:true,repeat:1,ease:'Quad.Out',onComplete:()=>this.defeatGoblin(station)})}
  defeatGoblin(station){const {goblin,corpse,goblinScale,attackIso}=station;if(!this.active||!this.regionActive||!goblin?.active)return;const fallAngle=station.spec.wall==='south'?68:-68;this.addTween({targets:goblin,y:goblin.y+12,angle:fallAngle,scaleX:goblinScale*.76,scaleY:goblinScale*.76,duration:430,ease:'Quad.In',onComplete:()=>{corpse.setIsoPosition(attackIso.u,attackIso.v,0).setVisible(true).setAlpha(.82).setAngle(fallAngle).setScale(goblinScale*.76).clearTint();goblin.setVisible(false).setAlpha(1).clearTint().setAngle(0).setScale(goblinScale);station.cycle++;this.schedule(650+station.index%3*140,()=>this.spawnGoblin(station))}})}
  ...
  setRegionActive(value){value=!!value;if(!this.active||this.regionActive===value)return;this.regionActive=value;this.clearRuntime();for(const station of this.stations){station.archerBob=null;station.goblinStride=null;station.archer.setVisible(value);station.goblin.setVisible(false).setAlpha(1).clearTint().setAngle(0).setScale(station.goblinScale);station.corpse.setVisible(false).setAlpha(1).setAngle(0);if(value){this.startArcherGuard(station);this.schedule(180+station.index*145,()=>this.spawnGoblin(station))}}}
