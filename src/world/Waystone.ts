...
export class Waystone extends Phaser.GameObjects.Container{
  constructor(scene,x,y,areaName,textureKey='waystone_dormant'){
    ...
    if(this.useSprite){
      ...
      // A identificação permanente poluía o cenário. O nome aparece apenas
      // no prompt contextual e no painel branco de interação.
      this.label=scene.add.text(0,this.cityWideArt?48:34,'',{fontFamily:'Arial',fontSize:11}).setOrigin(.5).setVisible(false);
      ...
      this.label=scene.add.text(0,38,'',{fontFamily:'Arial',fontSize:11}).setOrigin(.5).setVisible(false);
