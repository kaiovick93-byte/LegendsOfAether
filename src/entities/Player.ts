...
export class Player extends IsoPhysicsSprite{
  ...
  enableIsoMovement(config,x,y,z=0){this.isoDriven=true;this.configureIsoProjection(config);this.setIsoPosition(x,y,z);if(this.body){this.body.setVelocity(0,0);this.body.moves=false}return this}
  ...
  playMove(moving){if(this.dead)return;this.setFlipX(false);if(!moving){this.anims.stop();this.setFrame(this.getIdleFrame());return}const key=`${this.getTextureKey()}-walk-${this.facing}`;if(this.scene.anims.exists(key))this.anims.play(key,true);else this.setFrame(this.getIdleFrame())}
