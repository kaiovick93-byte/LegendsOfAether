...
export class Player extends Phaser.Physics.Arcade.Sprite{
  constructor(scene,x,y){super(scene,x,y,'player',1);scene.add.existing(this);scene.physics.add.existing(this);this.setScale(.7);this.setDepth(20);this.setCollideWorldBounds(true);this.body.setSize(58,58,true);this.hp=120;this.maxHp=120;this.mana=60;this.maxMana=60;this.level=1;this.xp=0;this.gold=25;this.attackDamage=14;this.defense=0;this.speed=170;this.characterClass='warrior';this.facing='down';this.dead=false;this.skillPoints=0;this.attributePoints=0;this.nextAttack=0;this.equipment={attack:0,defense:0,hp:0,mana:0,speed:0};this.skills={attack:0,hp:0,mana:0,speed:0}}
  ...
  gainXp(n){this.xp+=n;while(this.xp>=this.level*100){this.xp-=this.level*100;this.level++;this.attributePoints+=3;this.hp=this.maxHp;this.mana=this.maxMana;this.scene.sfx?.levelUp?.();this.scene.skillManager?.grant?.()}}
  ...
  allocateAttribute(stat){if(this.attributePoints<=0)return false;if(stat==='hp'){this.maxHp+=5;this.hp+=5}if(stat==='mana'){this.maxMana+=3;this.mana+=3}if(stat==='attack')this.attackDamage+=1;if(stat==='defense')this.defense+=1;this.attributePoints--;return true}
  serialize(){return{hp:this.hp,mana:this.mana,level:this.level,xp:this.xp,gold:this.gold,characterClass:this.characterClass,skillPoints:this.skillPoints,attributePoints:this.attributePoints}}
  loadState(s){this.level=s.level||1;this.xp=s.xp||0;this.gold=s.gold||0;this.skillPoints=s.skillPoints||0;this.attributePoints=s.attributePoints||0;this.applyClass(s.characterClass||'warrior');this.hp=Math.min(s.hp??this.maxHp,this.maxHp);this.mana=Math.min(s.mana??this.maxMana,this.maxMana);this.dead=false}
