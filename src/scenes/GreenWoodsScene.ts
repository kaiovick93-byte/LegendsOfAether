...
export class GreenWoodsScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.castleKey = this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.Y);
  ...
  update(): void {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.castleKey) && nearCastle) {
      this.requestSave();
    ...
    const nearCastle = Phaser.Math.Distance.Between(
      this.player.x,
      this.player.y,
      this.castleGateZone.x,
      this.castleGateZone.y
    ) < 36;

    if (nearCastle) {
      this.hintText.setText("Y: entrar no castelo | X: sair | E: coletar | I: inventário");
    }
  ...
  private createWorld(): void {
    ...
    //castle
    this.add.rectangle(52 * TILE_SIZE, 4 * TILE_SIZE, 84, 28, 0x5b4b76, 1)
      .setOrigin(0)
      .setDepth(1.5);

    this.castleGateZone = this.add.zone(52 * TILE_SIZE + 40, 4 * TILE_SIZE + 14, 56, 24);

    this.physics.add.existing(this.castleGateZone, true);
  }
  ...
  private createHUD(): void {
    ...
    this.hintText = this.add.text(
      GAME_WIDTH - 18,
      GAME_HEIGHT - 18,
      "Espaço: atacar | X: sair/entrar | E: entrar / coletar | I: inventário | R: equipar | H: cura | M: mana | Y: castelo",
      {
        fontFamily: "Arial",
        fontSize: "14px",
        color: "#c8d1ea"
