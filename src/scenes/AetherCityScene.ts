...
export class AetherCityScene extends Phaser.Scene {
  ...
  // A abertura física ocupa o vão entre as duas torres. Os antigos valores
  // deixavam menos de um corpo lógico de largura e fechavam o retorno.
  ...
  createWallsAndGates() {
    ...
    // A arte inteira do portão contém arco, céu e vão aberto. Registrá-la como
    // máscara alfa fechava a passagem. Somente as torres têm footprint físico.
  ...
  registerGateTowerFootprints() {
    ...
    const towers=[
      {id:'south-west-tower',u:12.15,v:26.03,label:'torre oeste do Portão Sul'},
      {id:'south-east-tower',u:15.85,v:26.03,label:'torre leste do Portão Sul'},
      {id:'east-north-tower',u:26.03,v:12.15,label:'torre norte do Portão Leste'},
      {id:'east-south-tower',u:26.03,v:15.85,label:'torre sul do Portão Leste'}
    ];
    for(const tower of towers){
      ...
      this.gateCollisionAnchors.push(anchor);
      this.registerSolidMask(anchor,'iso_city_wall',{
        label:tower.label,mode:'footprint',footprintWidth:62,footprintHeight:22,footprintYOffset:-10,
        originX:.5,originY:1,active:()=>anchor.active
      });
    }
  }
  ...
  createBuildings() {
    ...
    for (const building of this.getBuildingPlan()) {
      ...
      // O retângulo da planta descreve a base no chão e é a mesma geometria
      // que a futura navegação consultará. Telhados/transparências do PNG não
      // interferem mais na passagem nem deixam entradas acidentais pela frente.
      const [u,v,w,h]=building.rect;
      this.registerSolidMask(image, building.key, {
        label: building.label, mode: 'isoRect',
        isoRect:{u1:u+.16,v1:v+.16,u2:u+w-.16,v2:v+h-.16,corner:.30}
      });
  ...
  createPlazaAndStreets() {
    ...
    this.registerSolidMask(this.fountain, 'city_fountain', {
      // Apenas a bacia toca o chão; jatos e estátua continuam puramente visuais.
      label: 'fonte', mode:'footprint', footprintWidth:94,footprintHeight:30,footprintYOffset:-14
    });
    ...
    this.registerSolidMask(tree, 'city_tree', {
      label: 'tronco da árvore do Marco', mode:'footprint',footprintWidth:34,footprintHeight:16,footprintYOffset:-8
    });
  ...
  createEnvironmentalAccents() {
    ...
    for (const treeSpec of plan.trees) {
      ...
      this.registerSolidMask(tree, 'city_tree', {
        label: treeSpec.label, mode:'footprint',footprintWidth:30,footprintHeight:15,footprintYOffset:-7
      });
    }
  ...
  /**
   * Fonte única da planta lógica de um prédio. O mesmo teste é usado pelo
   * corpo do jogador e por consultas pontuais de rotas/atores, para que uma
   * futura navegação não "enxergue" uma geometria diferente do WASD.
   *
   * O retângulo é arredondado no espaço isométrico lógico: telhados e pixels
   * transparentes nunca entram nessa definição física.
   */
  isIsoRectBlocked(rect, u, v, radius = 0) {
    if (!rect) return false;
    const corner=Math.max(0,Math.min(
      Number(rect.corner) || 0,
      (rect.u2-rect.u1)/2,
      (rect.v2-rect.v1)/2
    ));
    const nearestU=Phaser.Math.Clamp(u,rect.u1+corner,rect.u2-corner);
    const nearestV=Phaser.Math.Clamp(v,rect.v1+corner,rect.v2-corner);
    const du=u-nearestU,dv=v-nearestV;
    return du*du+dv*dv<=(corner+Math.max(0,radius))**2;
  }
  ...
  getSolidMaskCollisionScore(u, v) {
    ...
    for (const entry of this.solidMasks) {
      if(entry.mode==='isoRect'){
        if(this.resolveSolidValue(entry.active,true)===false)continue;
        const rect=entry.isoRect;
        if(!rect)continue;
        if(this.isIsoRectBlocked(rect,u,v,this.playerIsoRadius??.27))totalHits+=100;
        continue;
      }
  ...
  isWorldPointBlockedBySolidMasks(x,y,ignoreOwner=null) {
    for(const entry of this.solidMasks||[]){
      ...
      if(entry.mode==='isoRect'){
        if(this.resolveSolidValue(entry.active,true)===false)continue;
        const iso=this.unproject(x,y),rect=entry.isoRect;
        if(this.isIsoRectBlocked(rect,iso.x,iso.y,0))return true;
        continue;
      }
      const geometry=this.getSolidGeometry(entry);
  ...
  createNpcs() {
    ...
    for (const [texture, name, role, u, v, pages, options] of specs) {
      ...
      const converted = options.iso && npc.setIsometricSprite?.(options.iso, {
        ...
        safeIdleBreathing:!!options.safeIdleBreathing,
  ...
  createWalker(texture, isoTexture, name, role, pages, logicalRoute, targetHeight, speed, delay, portrait, visualOptions={}) {
    ...
    const converted = npc.setIsometricWalkSprite?.(isoTexture, {height: targetHeight, facing: 'south', originY:visualOptions.originY});
    if (!converted) npc.setRealSprite?.(texture);
    npc.enableIsoPosition({
      tileWidth:AetherCityScene.TILE_WIDTH,
      tileHeight:AetherCityScene.TILE_HEIGHT,
  ...
  installAmbientAnimations() {
    ...
    ensure('elder-feed-birds', 'elder_feeder_iso_v3', [0,1,2,3], 2.4);
  ...
  createOldManAndBirdsIso() {
    ...
    if (this.textures.exists('elder_feeder_iso_v3')) {
      // A nova arte tem margem de segurança para cabeça/mãos, por isso a
      // escala parte da altura corporal real, não do canvas inteiro.
      const elderScale = 136 / 224;
      this.oldMan = new IsoSprite({
        scene:this,isoX:homeLogical.u,isoY:homeLogical.v,isoZ:0,texture:'elder_feeder_iso_v3',frame:0,
        tileWidth:AetherCityScene.TILE_WIDTH,tileHeight:AetherCityScene.TILE_HEIGHT,
        screenOriginX:AetherCityScene.ORIGIN_X,screenOriginY:AetherCityScene.ORIGIN_Y,
      ...
      this.registerSolidMask(this.oldMan,'elder_feeder_iso_v3',{
      ...
      console.warn('[AetherCity] elder_feeder_iso_v3 indisponível; cena mantida sem o velhinho.');
      this.oldMan = null;
    }
  ...
  setupHud() {
    ...
    if (this.waystone.sprite) {
      ...
      this.registerSolidMask(this.waystone.sprite, 'waystone_city_dormant', {
        label: 'Marco de Senda',mode:'footprint',footprintWidth:48,footprintHeight:16,footprintYOffset:-8,
