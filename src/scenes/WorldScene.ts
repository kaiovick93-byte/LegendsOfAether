...
export class WorldScene extends Phaser.Scene{
 ...
 createWorld(){
  ...
  this.safeRect={x:60,y:60,width:1040,height:760};this.drawCityGround();this.cityWalls=[];const makeWall=(x,y,w,h)=>{const wall=this.add.rectangle(x,y,w,h,0x000000,0).setOrigin(.5);this.physics.add.existing(wall,true);this.cityWalls.push(wall);this.physics.add.collider(this.player,wall)};makeWall(580,60,1040,20);makeWall(60,440,20,760);makeWall(245,820,370,20);makeWall(855,820,490,20);makeWall(1100,162.5,20,205);makeWall(1100,627.5,20,385);this.drawCityStructures();this.drawPlazaDecor();this.drawStreetDetails();this.applyCityVisualPolish();
  this.add.rectangle(60,60,1040,760,0x24314d,.025).setOrigin(0).setDepth(1);
  this.add.text(95,88,'CIDADE DE AETHER',{fontFamily:'Arial',fontSize:22,color:'#ecf0ff',fontStyle:'bold'}).setDepth(2);
  this.add.text(100,122,'ÁREA SEGURA • CIDADE',{fontFamily:'Arial',fontSize:13,color:'#73e6a8',fontStyle:'bold'}).setDepth(2);
  this.rightGate=this.add.zone(1100,350,56,170);this.bottomGate=this.add.zone(520,820,180,56);
 ...
 createOutskirtsLayout(){
  this.outskirtsRegions={farm:new Phaser.Geom.Rectangle(1220,1100,900,650),totems:new Phaser.Geom.Rectangle(2320,250,620,520),ruins:new Phaser.Geom.Rectangle(2550,1120,720,520),lake:new Phaser.Geom.Rectangle(1980,1260,700,520),cave:new Phaser.Geom.Rectangle(3240,1620,340,220),portal:new Phaser.Geom.Rectangle(3840,500,180,260)};
  this.add.rectangle(1120,50,this.worldWidth-1180,this.worldHeight-110,0x7b3d2a,.05).setOrigin(0).setDepth(0);
  ...
  this.stampPath(1114,350,1480,360,68);this.stampPath(1480,360,1810,520,64);this.stampPath(1810,520,2180,650,64);this.stampPath(2180,650,2520,640,60);this.stampPath(2520,640,2970,640,64);this.stampPath(2970,640,3500,650,64);this.stampPath(3500,650,3920,640,68);
  this.stampPath(520,820,760,960,60);this.stampPath(760,960,1080,1110,60);this.stampPath(1080,1110,1380,1250,64);this.stampPath(1380,1250,1640,1360,68);this.stampPath(1640,1360,1940,1440,62);this.stampPath(1940,1440,1970,1580,58);this.stampPath(1970,1580,2180,1680,60);this.stampPath(2180,1680,2470,1680,62);this.stampPath(2470,1680,2710,1510,58);
  ...
  [[1220,230],[1360,274],[1840,404],[1320,1000],[1740,1110],[2140,1160],[2280,980],[2760,820],[3220,700],[3520,560],[2030,1750],[1180,1560],[1510,1650],[2860,1710]].forEach(p=>herb(p[0],p[1],1.15));
  ...
  const streamNodes=[[1240,280,120,60],[1400,380,150,74],[1540,520,164,84],[1640,690,148,74],[1670,880,120,58],[1770,1050,148,70],[1930,1190,176,76],[2110,1290,158,72],[2280,1350,128,60]];
  ...
  [[1202,270,.52],[1300,313,.46],[1495,475,.50],[1588,647,.48],[1730,1010,.50],[1874,1152,.52],[2048,1252,.48],[2182,1322,.44]].forEach(p=>this.add.image(p[0],p[1],'outskirts_reeds').setScale(p[2]).setDepth(.25));
  [[1330,300,.42],[1590,570,.48],[1818,1115,.46],[2140,1188,.50]].forEach(p=>this.add.image(p[0],p[1],'outskirts_bush_cluster').setScale(p[2]).setDepth(.245));
  ...
  [[2860,760,92,60],[2960,700,84,56],[3070,760,110,66],[3200,860,120,74],[3330,920,92,56],[1470,550,72,46],[1540,590,64,40],[1180,1220,82,52]].forEach(r=>rock(...r));
  ...
  // Fazenda.
  this.add.text(1590,1110,'Fazenda de Rowan',{fontFamily:'Arial',fontSize:12,color:'#f4e2b6'}).setOrigin(.5).setDepth(.32);
  if(this.textures.exists('residential_house_red')){this.farmHouse=this.add.image(1450,1290,'residential_house_red').setOrigin(.5,1).setScale(.82).setDepth(3.05);this.createWorldBlock(1450,1218,128,76)}
  if(this.textures.exists('artisan_house')){this.farmShed=this.add.image(1638,1280,'artisan_house').setOrigin(.5,1).setScale(.64).setDepth(3.01);this.createWorldBlock(1638,1220,118,72)}
  if(this.textures.exists('chicken_coop')){this.farmCoop=this.add.image(1820,1470,'chicken_coop').setOrigin(.5,1).setScale(.34).setDepth(3.05);this.createWorldBlock(1820,1450,52,22)}
  // Plantações.
  for(let row=0;row<5;row++)for(let col=0;col<7;col++){const x=1710+col*24,y=1346+row*22;this.add.rectangle(x,y,18,12,0x6f5635,.54).setDepth(.2);for(let i=0;i<3;i++)this.add.ellipse(x-5+i*5,y-2,4,11,0x6ea653,.95).setDepth(.26)}
  this.add.rectangle(1782,1398,206,126,0x6b4b2d,.08).setDepth(.18).setStrokeStyle(2,0xae8e65,.18);
  // Cercas e currais.
  ...
  // Curral das vacas: cerca real nas bordas e abertura frontal de 86 px.
  fenceSpan(1520,1420,1710,1420,6);fenceSpan(1520,1420,1520,1515,3);fenceSpan(1710,1420,1710,1515,3);
  fenceSpan(1520,1515,1567,1515,2);fenceSpan(1663,1515,1710,1515,2);
  this.createWorldBlock(1615,1420,190,8);this.createWorldBlock(1520,1468,8,96);this.createWorldBlock(1710,1468,8,96);this.createWorldBlock(1543.5,1515,47,8);this.createWorldBlock(1686.5,1515,47,8);
  // Chiqueiro: abertura frontal de 70 px.
  fenceSpan(1750,1450,1890,1450,5);fenceSpan(1750,1450,1750,1525,3);fenceSpan(1890,1450,1890,1525,3);
  fenceSpan(1750,1525,1775,1525,1);fenceSpan(1865,1525,1890,1525,1);
  this.createWorldBlock(1820,1450,140,8);this.createWorldBlock(1750,1488,8,76);this.createWorldBlock(1890,1488,8,76);this.createWorldBlock(1762.5,1525,25,8);this.createWorldBlock(1877.5,1525,25,8);
  // Carroça e cavalo.
  this.add.rectangle(1364,1378,48,20,0x7a5433,.96).setDepth(.32).setStrokeStyle(2,0x9f7850,.6);this.add.rectangle(1364,1364,34,8,0x916845,.94).setDepth(.33);this.add.circle(1348,1394,7,0x393331,.96).setDepth(.31).setStrokeStyle(2,0xb19a76,.75);this.add.circle(1380,1394,7,0x393331,.96).setDepth(.31).setStrokeStyle(2,0xb19a76,.75);this.add.line(1414,1378,0,0,42,-6,0xc8ae7d,.84).setLineWidth(2).setDepth(.32);
  this.createWorldBlock(1364,1384,42,16);this.createWorldBlock(1450,1390,28,16);
 ...
 createOutskirtsAmbientLife(){
  ...
  this.farmCow1=this.add.image(1600,1470,'outskirts_cow').setOrigin(.5,.8).setScale(.68).setDepth(3.02);bob(this.farmCow1,2,1500);
  this.farmCow2=this.add.image(1664,1452,'outskirts_cow').setOrigin(.5,.8).setScale(.62).setDepth(3.01);bob(this.farmCow2,2,1360);
  this.farmPig1=this.add.image(1786,1490,'outskirts_pig').setOrigin(.5,.8).setScale(.70).setDepth(3.0);bob(this.farmPig1,1.5,1250);
  this.farmPig2=this.add.image(1842,1476,'outskirts_pig').setOrigin(.5,.8).setScale(.62).setDepth(3.0);sway(this.farmPig2,1.4,1180);
  this.farmHorse=this.add.image(1450,1382,'outskirts_horse').setOrigin(.5,.8).setScale(.62).setDepth(3.03);sway(this.farmHorse,1.2,1700);
  ...
  routeChicken('city_chicken_white','city-chicken-white-walk',1810,1490,[{x:1810,y:1490,pause:620},{x:1842,y:1482,pause:560},{x:1860,y:1504,pause:700},{x:1828,y:1512,pause:680}],300);
  routeChicken('city_chicken_brown','city-chicken-brown-walk',1840,1458,[{x:1840,y:1458,pause:580},{x:1868,y:1468,pause:680},{x:1852,y:1492,pause:720},{x:1822,y:1476,pause:640}],980);
 ...
 enforceCityBoundary(px,py){const x=this.player.x,y=this.player.y;const safe={left:60,right:1100,top:60,bottom:820};const eastGate=this.rightGate.getBounds();const southGate=this.bottomGate.getBounds();const wasInside=px>=safe.left&&px<=safe.right&&py>=safe.top&&py<=safe.bottom;const isInside=x>=safe.left&&x<=safe.right&&y>=safe.top&&y<=safe.bottom;if(wasInside!==isInside){const crossedEast=(px<safe.right&&x>=safe.right)||(px>safe.right&&x<=safe.right);const crossedSouth=(py<safe.bottom&&y>=safe.bottom)||(py>safe.bottom&&y<=safe.bottom);const allowed=(crossedEast&&(eastGate.contains(px,py)||eastGate.contains(x,y)))||(crossedSouth&&(southGate.contains(px,py)||southGate.contains(x,y)));if(!allowed)this.player.setPosition(px,py)}if(this.player.x<safe.left&&wasInside)this.player.x=safe.left;if(this.player.y<safe.top&&wasInside)this.player.y=safe.top;this.player.y=Math.min(this.player.y,this.worldHeight-120)}
 isSafeZone(){return this.player.x>=60&&this.player.x<=1100&&this.player.y>=60&&this.player.y<=820}
 ...
 tryShop(){const d=Phaser.Math.Distance.Between(this.player.x,this.player.y,this.merchant.x,this.merchant.y);if(d>80){this.showActionMessage('Aproxime-se de Aldren Voss para abrir a loja.');return}this.shop.open();this.hud.openExternalModal()}
