...
export class CastleScene extends Phaser.Scene {
  ...
  private castleExitZone!: Phaser.GameObjects.Zone;
  ...
  create(): void {
    ...
    this.combat = new CombatSystem(this, (enemy, player) => {
      ...
      player.addGold(20 + enemy.xpReward);
      this.requestSave();
    });
  ...
  update(): void {
    ...
    if (this.boss && this.boss.active) {
      ...
      if (!this.boss.isAlive() && !this.finalChestOpened) {
    ...
    const nearExit = Phaser.Math.Distance.Between(
      this.player.x,
      this.player.y,
      this.castleExitZone.x,
      this.castleExitZone.y
    ) < 42;

    const nearThrone = Phaser.Math.Distance.Between(
      this.player.x,
      this.player.y,
      this.throneRoomZone.x,
      this.throneRoomZone.y
    ) < 42;

    if (nearExit) {
      this.hintText.setText("X: voltar à floresta | E: coletar | I: inventário");
    } else if (nearThrone) {
      this.hintText.setText("E: abrir o baú real | X: voltar | I: inventário");
    } else {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.interactKey)) {
      if (nearThrone && this.finalChestOpened) {
        this.showMessage("O baú real já foi aberto.");
        return;
      }
  ...
  private createWorld(): void {
    ...
    for (let x = 0; x < WORLD.widthTiles; x++) {
      this.add.rectangle(x * TILE_SIZE, 0, 32, 32, 0x1d1a28, 1).setOrigin(0).setDepth(0.2);
      this.add.rectangle(x * TILE_SIZE, mapHeight - 32, 32, 32, 0x1d1a28, 1).setOrigin(0).setDepth(0.2);
    }

    for (let y = 0; y < WORLD.heightTiles; y++) {
      this.add.rectangle(0, y * TILE_SIZE, 32, 32, 0x1d1a28, 1).setOrigin(0).setDepth(0.2);
      this.add.rectangle(mapWidth - 32, y * TILE_SIZE, 32, 32, 0x1d1a28, 1).setOrigin(0).setDepth(0.2);
    }

    // corredores e colunas
    for (let x = 6; x < 54; x += 4) {
      for (let y = 6; y < 34; y += 6) {
    ...
    for (let x = 40; x < 54; x++) {
      for (let y = 8; y < 18; y++) {
        this.add.rectangle(x * TILE_SIZE, y * TILE_SIZE, 32, 32, 0x34233f, 0.2)
          .setOrigin(0)
          .setDepth(0.05);
      }
    }
    ...
    this.castleExitZone = this.add.zone(3 * TILE_SIZE + 24, 29 * TILE_SIZE + 10, 48, 24);
    this.physics.add.existing(this.castleExitZone, true);

    this.throneRoomZone = this.add.zone(49 * TILE_SIZE, 12 * TILE_SIZE, 80, 80);
    this.physics.add.existing(this.throneRoomZone, true);

    this.add.rectangle(45 * TILE_SIZE, 10 * TILE_SIZE, 110, 70, 0xc084fc, 0.12)
      .setOrigin(0)
      .setDepth(0.1);
  }

  private createColumn(tileX: number, tileY: number): void {
    const x = tileX * TILE_SIZE;
    const y = tileY * TILE_SIZE;

    this.add.rectangle(x, y, 30, 30, 0x4c3d59, 1).setOrigin(0).setDepth(1);

    const collider = this.add.rectangle(x + 2, y + 2, 26, 26, 0x000000, 0);
  ...
  private spawnEnemies(): void {
    this.enemies.push(
      new Enemy(this, 220, 160, "enemy-placeholder", "Royal Guard", {
        hp: 72,
        speed: 68,
        attackDamage: 16,
        aggroRange: 260,
    ...
    this.enemies.push(
      new Enemy(this, 340, 280, "enemy-placeholder", "Royal Guard", {
        hp: 72,
        speed: 68,
        attackDamage: 16,
        aggroRange: 260,
    ...
    this.enemies.push(
      new Enemy(this, 520, 220, "enemy-placeholder", "Shadow Knight", {
        hp: 98,
        speed: 74,
        attackDamage: 20,
        aggroRange: 280,
  ...
  private spawnFinalChest(): void {
    ...
    const chest = this.add.container(this.throneRoomZone.x, this.throneRoomZone.y);

    const base = this.add.rectangle(0, 8, 28, 16, 0x8f6a4a, 1);
    const lid = this.add.rectangle(0, 0, 30, 10, 0xffd166, 1);
    const label = this.add.text(20, -10, "Baú Real", {
      fontFamily: "Arial",
      fontSize: "12px",
      color: "#ecf0ff",
    ...
    chest.add([base, lid, label]);
    chest.setDepth(20);

    this.showMessage("O castelo foi purificado. Abra o baú com E.", "#ffd166");
  }
  ...
  private openFinalChest(): void {
    ...
    this.showMessage("Inventário cheio para receber a relíquia.", "#ff6b6b");
  }
  ...
  private handlePlayerDeath(): void {
    ...
    this.deathOverlay.show("Você perdeu 20 ouro e vai reaparecer.");

    const lostGold = Math.min(20, this.player.gold);
