 ...
 create(){
  ...
  for(const [classId,x] of Object.entries(classX)){
   ...
   ids.forEach((id,index)=>{
    ...
    const panel=this.add.rectangle(cx,214,122,230,0x182033,.98).setStrokeStyle(2,0x32405f,1).setInteractive({useHandCursor:true});
    ...
    const gender=this.add.text(cx,292,appearance.genderLabel.toUpperCase(),{fontFamily:'Arial',fontSize:'11px',color:'#ecf0ff',fontStyle:'bold'}).setOrigin(.5);
    const heritage=this.add.text(cx,310,appearance.heritage,{fontFamily:'Arial',fontSize:'10px',color:'#9aa8c7'}).setOrigin(.5);
    const choose=()=>this.choose(id);
    panel.on('pointerdown',choose);portrait.on('pointerdown',choose);gender.setInteractive({useHandCursor:true}).on('pointerdown',choose);heritage.setInteractive({useHandCursor:true}).on('pointerdown',choose);
