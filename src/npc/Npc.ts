...
export class Npc extends IsoContainer{
 ...
 setIsometricSprite(textureKey:string,options={}){
  ...
  // Alguns personagens possuem gesto em folha própria; outros (como o
  // General) precisam de uma respiração segura quando a folha de gesto não
  // mantém todos os membros no mesmo desenho.
  this.isoSafeBreathing=!!options.safeIdleBreathing;
 ...
 setIsometricWalkSprite(textureKey:string,options={}){
  ...
  this.sprite=this.scene.add.sprite(0,8,textureKey,0).setOrigin(.5,originY);
  const frame=this.scene.textures.get(textureKey).get(0);
  const targetHeight=options.height??110;
  // A altura lógica é medida até a linha dos pés; células com padding de
  // segurança não reduzem nem fazem o caminhante flutuar.
  this.isoDisplayScale=targetHeight/((frame?.height||224)*originY);
 ...
 playIsometricIdleAction(profile){
  if(this.isoSafeBreathing){
   const baseScale=this.isoDisplayScale||this.sprite.scaleY||1;
   this.sprite.setTexture(this.isoBaseTexture,0).setPosition(0,8).setAngle(0).setScale(baseScale,baseScale);
   this.idleActionTween=this.scene.tweens.add({
    targets:this.sprite,
    scaleY:{from:baseScale*.992,to:baseScale*1.008},
    duration:760,ease:'Sine.InOut',yoyo:true,
    onComplete:()=>{
     if(this.sprite?.active)this.sprite.setScale(baseScale,baseScale).setTexture(this.isoBaseTexture,0);
