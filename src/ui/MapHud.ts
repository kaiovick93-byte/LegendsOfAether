...
export class MapHud{
 constructor(scene,opts){
  ...
  const mapOptions={
   ...
   logicalBounds:opts.logicalBounds||null,
   artKey:opts.mapTexture||null,projection:opts.mapProjection||'world',onClose:()=>this.closeAll()
  };
  this.minimap=new Minimap(scene,opts.worldWidth,opts.worldHeight,opts.localName,{artKey:mapOptions.artKey,projection:mapOptions.projection,logicalBounds:mapOptions.logicalBounds});
  (opts.minimapMarkers||opts.markers||[]).forEach(marker=>this.minimap.addMarker(marker));
 ...
 refreshMapMarkers(){this.minimap.refreshMarkerVisibility?.();this.mapPanel.refreshMarkerVisibility?.()}
 update(){
