...
export class BottomActionBar {
  constructor(private scene, private player, private abilities, private getItemCount, private onAction=()=>{}){
    ...
    for(let index=0;index<slotCount;index++){
      ...
      if(special){
        const icon=scene.add.image(x,slotY-6,iconTextures[index]).setDisplaySize(28,28);
        ...
        back.on('pointerover',()=>back.setStrokeStyle(2,0xe0bd72,1));
        back.on('pointerout',()=>back.setStrokeStyle(1.35,0xb28a45,.96));
  ...
  update(){
    ...
    this.slotTexts[12].setText('MAPA');
    this.slotTexts[13].setText('CONTROLES');
    this.slotTexts[14].setText('MENU');
  }
