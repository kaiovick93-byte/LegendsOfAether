...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'aether-round66-'));
const canvas=path.join(temp,'canvas.miff');
const output=path.join(root,'ROUND66_CITY_VISUAL_QA.png');
...
const buildings=[
 ...
 {id:'artisan',key:'artisan_house',u:13.40,v:18.40,height:230,rect:[11.70,16.85,3.40,3.00]},
...
const addWallRun=(fixedAxis,fixed,start,end,flip)=>{
 ...
 const step=(end-start)/count,visualLength=step+.24;
 for(let index=0;index<count;index++){
  const cursor=start+index*step,next=start+(index+1)*step,middle=(cursor+next)/2;
  const u=fixedAxis==='u'?fixed:middle,v=fixedAxis==='v'?fixed:middle,p=project(u,v);
  items.push({relative:'assets/images/environment/isometric/isometric_city_wall.png',width:Math.round(visualLength*56+42),height:Math.round(visualLength*30+126),x:Math.round(p.x),y:Math.round(p.y+52),originY:.79,flip,depth:p.y+80});
 }
};
...
const npcs=[
 ['merchant_iso',6.58,13.87,112],['scholar_iso',6.81,8.44,110],['blacksmith_iso',10.71,6.09,114],
 ['healer_iso',14.52,7.13,112],['tavernkeeper_iso',19.80,6.90,114],['artisan_iso',13.48,18.91,112],
 ['elder_mira_iso',16.90,13.05,112],['guard_iso',22.8,17.1,116],['south_guard_iso',17.8,23.4,116]
];
